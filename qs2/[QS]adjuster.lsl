...
// v2 FORK ONLY. [QS]menu carries no authoring literals, so we register
// our own buttons. The channel field is a LINK MESSAGE number, not a
// chat channel: 90101 is the back route this script already handles, so
// the click arrives in the existing handler unchanged.
integer QSADJ_REGISTER   = 90213;
integer QSADJ_UNREGISTER = 90216;
integer QSADJ_CLICKNUM   = 90101;
...
default
{
    state_entry()
    {
        ...
        // v2 FORK ONLY. [QS]menu carries no authoring literals, so the
        // two buttons v1's sitB hardcodes have to be registered like any
        // other plugin's. Flags bit 0 = owner-only, which the adjust ACL
        // then widens exactly as before.
        //
        // This is also what finally makes /5 cleanup complete: in v1 the
        // buttons are rendered by sitB and survive our removal, which the
        // comment at the qs:alive delete below describes as an observed
        // bug on the Paloma sofa. Registered entries leave with their
        // owner.
        llMessageLinked(LINK_SET, QSADJ_REGISTER,
            "[HELPER]|" + (string)QSADJ_CLICKNUM + "|" + llGetScriptName() + "|1", "");
        llMessageLinked(LINK_SET, QSADJ_REGISTER,
            "[HELPER HUD]|" + (string)QSADJ_CLICKNUM + "|" + llGetScriptName() + "|1", "");
    ...
    listen(integer chan, string name, key id, string msg)
    {
        if (chan == chat_channel)
        {
            if (msg == "cleanup")
            {
                ...
                // v2 FORK ONLY. Drop our registered entries before
                // vanishing. PROTOCOL.md is explicit that a creator tool
                // which self-deletes MUST send this first, or its button
                // outlives it and dispatches to a channel nobody listens
                // on. Link messages are queued, so this is delivered even
                // though the next line removes us.
                llMessageLinked(LINK_SET, QSADJ_UNREGISTER, llGetScriptName(), "");
                llRemoveInventory(llGetScriptName());
            }
            else if (msg == "targets")
            {
