...
| `90215` | same free band | `[QS]adjuster` → creator-only plugins: `QS_FINALIZE` — broadcast by the `/5 cleanup` finalization right before the adjuster removes itself. Subscribers tear THEMSELVES down (the adjuster knows no Pro-kit inventory names, repo split); the owner-typed command counts as the confirmation. Current subscribers: `[QS]animesh` (external repo, runs its `[FINALIZE]` teardown without the dialog), `[QS]animeshRemote` (external repo, unpairs from the AVpos editor and drops its `[ADJUST]` entry first) and `[QS]AVpos-shifter` ([qs/tools/](./tools/), removes itself; it registers no `[ADJUST]` entry, so it owes no `90216`). Unheard when no such plugin is present. |
...
| `90213` | same free band | plugin → `[QS]sitB`: `QSADJ_REGISTER` — register a button into the `[ADJUST]` submenu (not `[OPTIONS]`). msg = `<label>\|<click_chan>\|<scriptName>\|<flags>`, id = `""`. `flags` bit 0 = owner-only (render + dispatch gated to `llGetOwner()`, like `[QUICKYHUD]`). sitB dedupes by scriptName+label (since 1.261 — one script may own several entries; before that, by scriptName alone); click dispatched to `<click_chan>` with the same `<controller>\|<sitter>` composite-id rule as the notecard `ADJUST` line. sitB ≥ 1.04. Removal goes through `90216` (`QSADJ_UNREGISTER`). See [§ QSADJ_REGISTER](#qsadj_register--dynamic-adjust-submenu). |
| `90279` | same free band | plugin → `[QS]sitB`: `QSDLG_RENDER` — render a dialog to the addressed sitter. msg = `<chan>\|<text>\|<b0>\|…\|<b11>` (14 pipe fields; text and labels must not contain `|`, and labels must be NON-EMPTY — sitB hands them to `llDialog` unfiltered, which refuses empty button strings)\|`), id = the sitter's key; exactly the sitB copy whose seat holds that sitter calls `llDialog`. Exists because `llDialog` force-sleeps the CALLING script 1.0 s: a plugin serving several sitters from one script serializes them behind each other's sleeps, while sitB is one copy per seat — a sleep lane per sitter. Same-script rendering also makes the new window REPLACE the sitter's previous sitB window instead of stacking. The reply is agent chat on `<chan>`; the plugin keeps its own listener, sitB renders and forgets. First consumer: `[QS]huddialog` (external repo). sitB ≥ 1.263. |
| `90216` | same free band | plugin → `[QS]sitB`: `QSADJ_UNREGISTER` — drop a previously registered `[ADJUST]` entry. msg = `<scriptName>`; since sitB 1.261 this removes EVERY entry that script registered, not one, id = `""`. Counterpart to `90213`, needed because `ADJUST_DYN` is a RAM registry with no other removal path: it only empties on a sitB reset. A creator-only tool that self-deletes on `QS_FINALIZE` (`90215`) MUST send this first, otherwise its button outlives it as a dead entry that dispatches to a channel nobody listens on and keeps the self-show `[ADJUST]` alive on a piece with `AMENU` off. Sent `LINK_SET` (sitB may be in another prim; each instance drops its own copy) and delivered even when the sender removes itself in the same event, since link messages are queued. sitB ≥ 1.2563; unknown scriptName and empty msg are no-ops. |
...
(label, click channel, scriptName). Since sitB 1.261 the dedupe identity is scriptName+label, so one script may own several entries, and QSADJ_UNREGISTER removes ALL of a script's entries at once. Field 3 is new:
...
scriptName+label (1.261). `adjust_dialog` renders the notecard `ADJUST_MENU` entries
