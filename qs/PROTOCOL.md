...
| `90034` | between stock `90033` and `90045` | `[QS]sitB` → `[QS]sitA`: post-quiet-swap menu reopen request. msg `""`, id = the swapped-out sitter's key. Sent by sitB's `90031` swap handler **only when that sitter had a pose dialog open** (`dialog_open_for == MY_SITTER`). `[QS]sitA` stores it in `forceReopenKey`; `run_time_permissions` then reopens the menu for the matching `PermissionsKey` despite `bSilentSwap`, replacing the now-dead dialog (listener removed) with a fresh working one on the destination slot. Users with no dialog open get no `90034`, hence no surprise menu. `forceReopenKey` is reset at the top of every `90030`/`90031` handler (90034 is causally later than the swap broadcast, so the reset always precedes the set) and consumed in `run_time_permissions`. |
