...
| `qs:hud:authoring` | `"locked"` (any other value or absent = open): creator authoring lock (1.255). Singleton, seeded from the AVpos `AUTHORING locked` line, which is the single source of truth (threat model: whoever can edit the AVpos on modify furniture can replace the whole script set anyway, so the lock aims at the menu surface, not at scripted attackers). `"locked"` removes the whole authoring surface (`[HELPER]`/`[QUICKYHUD]`, ADJUSTMODE, `[NEW]`/`[DUMP]`/`[SAVE]`, the ADJUSTMODE default-persist write) for everyone including the OWNER; the adjust ACL below can never override it. `start_boot` clears the key at parse start (it sits under `qs:hud:`, outside the `^qs:(meta\|cfg\|sitter\|p\|nm\|nt\|boot):` wipe pattern, and hudadmin's other `qs:hud:*` keys must survive), so removing the line + re-saving the notecard unlocks; after an `llLinksetDataReset` the normal re-seed restores it with everything else. No cross-repo readers: since hudproxy 1.256 removed the Switch-ADJUSTMODE settings entry, ADJUSTMODE's only entry is the sitter-side `[QUICKYHUD]` flow, so the HUD scripts carry no lock code. See [PROTOCOL.md § Authoring lock](./PROTOCOL.md). | boot (AVpos `AUTHORING` token) | sitB + adjuster `authoring_locked()` |
