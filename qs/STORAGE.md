...
| `qs:hud:standing` | `<owner uuid>` while a Remote-authoring standing session is active on this piece (see [PROTOCOL.md § Remote authoring](./PROTOCOL.md)); absent otherwise. Written by hudproxy's session open, deleted by every teardown path AND defensively in hudproxy's init — a script reset kills the listeners, so no session survives one and the flag must not either. External writer (hudproxy, sibling repo). | hudproxy (external repo) | adjuster `/5 animesh off` gate (only a piece with an active session reacts) |
