...
| `qs:hud:authoring` | `"open"` / `"locked"`: creator authoring lock (1.26). Singleton. `"locked"` removes the whole authoring surface (`[HELPER]`/`[QUICKYHUD]`, ADJUSTMODE, `[NEW]`/`[DUMP]`/`[SAVE]`, the ADJUSTMODE default-persist write) for everyone including the OWNER; `"open"` allows it (subject to the adjust ACL below); ABSENT fails closed when the QuickyHUD pipeline is present (readers probe `QPP_CFG:ADJUSTMODE` key existence), open otherwise (plain OSS furniture keeps stock behavior). External writer: hudadmin in the private [quicky-hud](https://github.com/QuickyProducts/QuickyHUD) repo, license-gated; it mirrors a protected HUD-side anchor key (`QPP_CFG:AUTHORING`, pass private to that repo) and re-arms both keys from RAM in its `linkset_data` handler, so an `llLinksetDataReset()` or a delete/forge of this plain key degrades toward LOCKED, never toward unlocked. Deliberately NOT reset on `CHANGED_OWNER` (the lock must survive the sale; contrast `qs:sec:adjust`). Not wiped by boot's re-seed (outside the `^qs:(meta\|cfg\|sitter\|p\|nm\|nt\|boot):` pattern). | hudadmin (external repo) | sitB `authoring_locked()`, adjuster `authoring_locked()` |
