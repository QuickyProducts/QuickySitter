...
| `qs:hud:standing` | `<owner uuid>` while a Remote-authoring standing session is active on this piece (see [PROTOCOL.md § Remote authoring](./PROTOCOL.md)); absent otherwise. Written by [QS]animeshAuthoring on session start and deleted on every teardown (a script reset kills its listeners, so no session survives one). External writer (sibling repo). |  [QS]animeshAuthoring (external repo) | adjuster `/5 animesh off` gate (only a piece with an active session reacts) |
| `qs:dump:for` | key of whoever pressed the standing [DUMP]; the finished link goes to this key, plus the owner when that is someone else. Handed over in LSD because boot compares the 90098 id verbatim against "quiet", so the payload could not grow without breaking version skew. Absent (old sender, seated path, failed write) means owner, the pre-1.28 behaviour. Consumed (deleted) at dump start, so it can never redirect a later run. | [QS]animeshAuthoring (external repo), written immediately before its 90098 | boot 90098 handler (read + delete) |
| `qs:alive:security` | `"1"` while [QS]root-security is in the linkset; absent otherwise (1.2512). Same contract as the other `qs:alive:*` flags: written in `state_entry`, wiped by boot's `QS_ALIVE_CENSUS` (90079) and re-stamped by the survivor's census handler, so a removed plugin stays cleared. Added because root-security was the only plugin publishing no presence, which forced readers onto the 90201/90202 event mirror (`has_security`), a cached reply that reads FALSE until the probe round trip completes. See [PROTOCOL.md § qs:alive](./PROTOCOL.md). | root-security `state_entry` + `QS_ALIVE_CENSUS` handler | available to any script needing synchronous security-plugin presence (the `has_security` mirror in sitA/sitB/adjuster is unchanged) |
| `qs:sec:adjust` | `"OWNER"` / `"GROUP"` / `"ALL"` — adjust-access ACL level (who may enter the adjust workflows: `[HELPER]`/`[QUICKYHUD]` + owner-gated QSADJ_REGISTER entries). Singleton. Owner-managed via [QS]root-security's `[SECURITY]` → `Adjust` menu (entry rendered only while `qs:alive:adjuster` is set, 1.2511) or via [QS]adjuster's `/5 adjust owner|group|all` chat command (1.2556; since adjuster 1.2559 it writes this key AND sends 90204 `QSADJ_ACL_SET` unconditionally, so root-security's RAM `ADJUST_INDEX` cannot go stale and revert the level on the next census). Deliberately under its own `qs:sec:` prefix so boot's re-seed wipe (`^qs:(meta\|cfg\|sitter\|p\|nm\|nt\|boot):`) does NOT clear it; after a full `llLinksetDataReset` root-security re-writes it on `QS_ALIVE_CENSUS` (90079) and on its `state_entry`; on `CHANGED_OWNER` it resets to `"OWNER"` (a sale must not carry the previous creator's widened ACL to the buyer); [QS]adjuster additionally deletes the key on its own `CHANGED_OWNER`, covering furniture without the security plugin. Readers treat an absent key — or neither writer present (`has_security == FALSE` AND `qs:alive:adjuster` unset) — as `"OWNER"` (pre-1.25 behavior). | root-security `write_adjust_access()`, adjuster `/5 adjust` | sitB `adjust_allowed()` (render + dispatch gates), adjuster `adjust_allowed()` (`[HELPER]`/`[QUICKYHUD]` click gates) |
