...
| `qs:sec:adjust` | `"OWNER"` / `"GROUP"` / `"ALL"` — adjust-access ACL level (who may enter the adjust workflows: `[HELPER]`/`[QUICKYHUD]` + owner-gated QSADJ_REGISTER entries). Singleton. Owner-managed via [QS]root-security's `[SECURITY]` → `Adjust` menu (entry rendered only while `qs:alive:adjuster` is set, 1.2511) or via [QS]adjuster's `/5 adjust owner|group|all` chat command (1.2556; routed through root-security via 90204 `QSADJ_ACL_SET` when present, direct LSD write otherwise). Deliberately under its own `qs:sec:` prefix so boot's re-seed wipe (`^qs:(meta\|cfg\|sitter\|p\|nm\|nt\|boot):`) does NOT clear it; after a full `llLinksetDataReset` root-security re-writes it on `QS_ALIVE_CENSUS` (90079) and on its `state_entry`; on `CHANGED_OWNER` it resets to `"OWNER"` (a sale must not carry the previous creator's widened ACL to the buyer); [QS]adjuster additionally deletes the key on its own `CHANGED_OWNER`, covering furniture without the security plugin. Readers treat an absent key — or neither writer present (`has_security == FALSE` AND `qs:alive:adjuster` unset) — as `"OWNER"` (pre-1.25 behavior). | root-security `write_adjust_access()`, adjuster `/5 adjust` | sitB `adjust_allowed()` (render + dispatch gates), adjuster `adjust_allowed()` (`[HELPER]`/`[QUICKYHUD]` click gates) |
