...
| `qs:hud:authoring` | `"locked"` (any other value or absent = open): creator authoring lock (sitter enforcement 1.255, trigger since hudadmin 1.258). Singleton, mirrored by hudadmin (external [quicky-hud](https://github.com/QuickyProducts/QuickyHUD) repo) from the `AUTHORING locked` keyword line in the Pro kit's `hudconfig` notecard, which is the single source of truth (threat model: whoever can edit the notecards on modify furniture can replace the whole script set anyway, so the lock aims at the menu surface, not at scripted attackers). `"locked"` removes the whole authoring surface (`[HELPER]`/`[QUICKYHUD]`, ADJUSTMODE, `[NEW]`/`[DUMP]`/`[SAVE]`, the ADJUSTMODE default-persist write) for everyone including the OWNER; the adjust ACL below can never override it. `"open"`/no line/no hudconfig deletes the key (no stale state, removing the line unlocks WITHOUT a pose re-seed; boot does not touch this key). After an `llLinksetDataReset` hudadmin's `linkset_data` handler re-writes it from config RAM; after a combined script + store wipe `readConfig` restores it from the hudconfig. Furniture without the QuickyHUD pair has no lock feature (deliberate, Pro-kit feature per the license split). Since hudproxy 1.256 ADJUSTMODE's only entry is the sitter-side `[QUICKYHUD]` flow. See [PROTOCOL.md § Authoring lock](./PROTOCOL.md). | hudadmin (hudconfig `AUTHORING` line, external repo) | sitB + adjuster `authoring_locked()` |
