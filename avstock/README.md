...
[`qs/lsl/`](../qs/lsl/) under their `[QS]…` names. This directory exists
...
so the cross-references in `qs/lsl/PROTOCOL.md` and `qs/lsl/STORAGE.md`
...
├── avsitter2_link_message_reference.md  ← spec, target of qs/lsl/PROTOCOL.md
├── Plugins/
...
Forks (the `qs/lsl/[QS]…` files) follow QuickySitter's trademark posture
...
If a forked-into-`qs/lsl/` script changed upstream, **don't** copy the
